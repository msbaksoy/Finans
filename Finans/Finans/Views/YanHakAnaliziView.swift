...
struct YanHakAnaliziView: View {
    ...
    /// Gemini API anahtarı (varsayılan entegre). Git push öncesi koddan kaldırın.
    @AppStorage("gemini_api_key") private var geminiAPIKey = "AIzaSyCJdtERkLNrG2DBUSCU5FwEGqdrJEue5ew"
    @State private var geminiAPIKeyGecici = ""
                ...
                if geminiAPIKey.isEmpty {
                    VStack(alignment: .leading, spacing: 8) {
                        Text("Gemini API Anahtarı")
                            .font(.subheadline.weight(.medium))
                            .foregroundColor(appTheme.textSecondary)
                        Text("AI yorumu almak için Google AI Studio'dan ücretsiz API anahtarı alın: aistudio.google.com")
                            .font(.caption)
                            .foregroundColor(appTheme.textSecondary)
                        SecureField("API anahtarınızı girin", text: $geminiAPIKeyGecici)
                            .textContentType(.password)
                            .autocapitalization(.none)
                            .padding(12)
                            .background(RoundedRectangle(cornerRadius: 10).fill(appTheme.cardBackgroundSecondary))
                        Button("Kaydet") {
                            geminiAPIKey = geminiAPIKeyGecici.trimmingCharacters(in: .whitespacesAndNewlines)
                        }
                        .font(.subheadline.weight(.semibold))
                        .foregroundColor(temaRengi)
                    }
