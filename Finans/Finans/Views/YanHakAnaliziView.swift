...
struct YanHakAnaliziView: View {
    ...
    /// Groq API anahtarı (console.groq.com). Lütfen Ayarlar'dan ekleyin (varsayılan boş).
    ...
    // Simplified result view shown when flow completes: only chart + comparison
    private var simplifiedResultView: some View {
                    ...
                    HStack(alignment: .top) {
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            Text("Aylık Fark")
                                .font(AppTypography.subheadline)
                                .foregroundColor(appTheme.textSecondary)
                            let delta = (teklif.yillikNetMaas - mevcutIs.yillikNetMaas) / 12.0
                            Text("\(delta >= 0 ? "+" : "")\(FinanceFormatter.currencyString(delta))")
                                .font(AppTypography.amountLarge)
                                .monospacedDigit()
                                .foregroundColor(delta >= 0 ? Color(hex: "16A34A") : Color(hex: "EF4444"))
                        }
                    }
                }
                .padding(16)
