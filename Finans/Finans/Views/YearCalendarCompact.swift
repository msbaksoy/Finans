...
/// Tekil, tekrar kullanılabilir resmi tatil takvimi (2026–2028).
/// BrutNet ekranı ve ileride diğer modüller tarafından kullanılır.
struct YearCalendarCompact: View {
    ...
    var body: some View {
        ...
        .alert(isPresented: $showHolidayAlert) {
            Alert(
                title: Text("Tatil Bilgisi"),
                message: Text(selectedHolidayText ?? ""),
                dismissButton: .default(Text("Kapat"))
            )
        }
    }
}
